...
if len(sys.argv) > 2:
    fusion_freq=int(sys.argv[2])
else:
    fusion_freq=1

session_target = sys.argv[3] if len(sys.argv) > 3 else None
...
if __name__ == "__main__":
    ...
    data_owners = [
        DataOwner(
            "data-owner-0", "./MNIST_DATA/4Parties/data_party0.npz", model_owner.build_update_step, model_owner.reset_dataonwer_model
        ),
        DataOwner(
            "data-owner-1", "./MNIST_DATA/4Parties/data_party1.npz", model_owner.build_update_step, model_owner.reset_dataonwer_model
        ),
        DataOwner(
            "data-owner-2", "./MNIST_DATA/4Parties/data_party2.npz", model_owner.build_update_step, model_owner.reset_dataonwer_model
        ),
        DataOwner(
            "data-owner-3", "./MNIST_DATA/4Parties/data_party3.npz", model_owner.build_update_step, model_owner.reset_dataonwer_model
        ),
    ]
    ...
    with tfe.Session(target=session_target) as sess:
        ...
        for i in range(model_owner.ITERATIONS*fusion_freq):
            # print('i',i)
            iter_each_epoch=60000//128//4*fusion_freq
